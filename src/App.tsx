...
import logo from "figma:asset/ab71d6e472d2ac6c6448b06fcfa0a9e87d3efd13.png";
import profilePic from "figma:asset/cf59af76f0b8abc91be0ffafe2097114d95c8349.png";
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setStats(prev => ({
        ...
        avgLatency: Math.max(30, Math.min(60, prev.avgLatency + Math.floor(Math.random() * 6) - 3)),
        fairnessScore: Math.max(85, Math.min(98, prev.fairnessScore + Math.floor(Math.random() * 3) - 1)),
        avgAccuracy: Math.max(80, Math.min(95, prev.avgAccuracy + (Math.random() * 2 - 1)))
      }));

      // Update core performance stats
      setCorePerformanceStats(prev => ({
        score: prev.score + Math.floor(Math.random() * 150),
        accuracy: Math.max(75, Math.min(95, prev.accuracy + (Math.random() * 4 - 2))),
        reactionTime: Math.max(180, Math.min(350, prev.reactionTime + Math.floor(Math.random() * 20 - 10))),
        targetsHit: prev.targetsHit + Math.floor(Math.random() * 3),
        targetsMissed: Math.max(0, prev.targetsMissed + Math.floor(Math.random() * 2 - 0.5)),
        emptyClicks: Math.max(0, prev.emptyClicks + Math.floor(Math.random() * 2 - 0.5))
      }));
      ...
      const eventTypes = [
        ...
        { event: 'Minor latency fluctuation detected', severity: 'warning' as const },
        { event: 'Network connection stabilized', severity: 'info' as const }
      ];
      const randomEvent = eventTypes[Math.floor(Math.random() * eventTypes.length)];
      const players = ['ProGamer123', 'SkillMaster', 'CasualGamer', 'CompPlayer99'];
        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                  <PlayerSessionsTable sessions={sessions} onPlayerClick={handlePlayerClick} />
                </div>
                <div>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              ...
              <div className="grid lg:grid-cols-2 gap-6 mt-6">
                <FairnessMetrics metrics={fairnessMetrics} overallScore={stats.fairnessScore} />
          ...
          {activeTab === 'live' && (
            <div className="space-y-6">
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                  <PlayerSessionsTable sessions={sessions.filter(s => s.status === 'active')} onPlayerClick={handlePlayerClick} />
                </div>
                <div className="space-y-6">
                  <LiveDataFeed events={liveEvents} />
                  <PartyWidget sessions={sessions} />
          ...
          {activeTab === 'player' && selectedPlayer && (
            <div className="space-y-6">
              <PlayerAnalytics player={selectedPlayer} sessions={sessions} onBack={handleBackToSessions} onPlayerChange={handlePlayerChange} />
            </div>
          )}
        </div>
      </div>
